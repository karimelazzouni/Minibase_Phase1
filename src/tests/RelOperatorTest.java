...
class RelOperatorTest extends TestDriver {
  ...
  protected boolean test3() {
    try {
      ...
      saveCounts(null);
      Predicate preds1 [] = new Predicate[] { new Predicate(AttrOperator.EQ,
              AttrType.FIELDNO, 0, AttrType.FIELDNO, 3) };
      Predicate preds2 [] = new Predicate[] { new Predicate(AttrOperator.EQ,
              AttrType.FIELDNO, 2, AttrType.FIELDNO, 6) };
      
      SimpleJoin join1 = new SimpleJoin(new FileScan(s_groups, groups), 
    		  new FileScan(s_rides, rides), preds1);
      SimpleJoin join2 = new SimpleJoin(join1, new FileScan(s_drivers, drivers), preds2);
