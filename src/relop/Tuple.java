...
public class Tuple {
   ...
   * allocates a new page (initializes array of byte) 
   * with length equivalent to the schema's length
   * 
   * schema.getLength() returns the sum of all its fields' lengths
   * 
  ...
  public Tuple(Schema schema, Object... values) {
    this.schema = schema;
    data = new byte[schema.getLength()];
    setAllFields(values); // eventually writes bytes into page
  }
